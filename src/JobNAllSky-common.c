...
// Number of degrees of freedom for Chi^2 (lvcvirgo.c)
extern int nd;

extern double dt, *t2; 

extern FILE *joblist;
extern char jobname[64];
...
JobCore(int pm,			// hemisphere 
	...
	int nmin,		// nmin+1: first point of the F-statistic to search for a signal
	int nmax,		// nmax: last point of the F-statistic to search for a signal
	double sepsm,		// sin(epsm)
	double cepsm,		// cos(epsm)
	double sphir,		// sin(phi_r)
	double cphir,		// cos(phi_r)
	double *signal,		// reference to array with the parameters of the candidate signal 
	int *sgnlc,		// counter of current signals in array
...
*/ 
	...
	// if not, returns NULL 
	if((sqr(al1)+sqr(al2))/sqr(coft) > 1.) 
	  {
	    if (write_st)
		{
			joblist = fopen(jobname,"a");
			fprintf (joblist,"\nOut of Sky in Jobcore mm=%d nn=%d\n", mm, nn );
			fclose (joblist);
		}
		return 1;//NULL ; 
	}
    ...
    /* amplitude modulation functions */
    modvir (sinalt, cosalt, sindelt, cosdelt, sphir, cphir, aa, bb, Npoints);
    ...
    for (ss=smin; ss<=smax; ss++) {
		...
		if (sgnlt[1] >= -Smax && sgnlt[1] <= 0.) {
			...
			for (i=nmin; i<nmax; i++)
					F[i] = sqr (creal(xao[i])) + sqr (cimag(xao[i])) + sqr (creal(xbo[i])) + sqr (cimag(xbo[i]));
			...
			for (i=nmin; i<nmax; i++) {
				if ((Fc = F[i]) > trl) {
					...
					for (j=0; j<NPAR; j++) signal[NPAR*(*sgnlc)+j] = sgnlt[j];
					(*sgnlc)++;
					if (write_st) {
						joblist = fopen(jobname,"a");
						fprintf (joblist,".");
